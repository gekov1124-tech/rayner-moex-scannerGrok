...
В `config.yaml` параметр `universe`:
- `sample` — 25 акций
- `blue` — голубые фишки
- `futures` — только фьючерсы FORTS (Si, RTS, MIX, Brent, Gold…)
- `mixed` — акции + фьючерсы (рекомендуется)

Контракты фьючерсов программа выбирает сама (ближайший по открытому интересу).


## Мониторинг и графики

После деплоя откройте URL Railway в браузере:
- Главная: статус сессии + кнопка «Сканировать сейчас»
- /chart/SBER — график с Entry/Stop
- В торговые часы (10:00–18:50 МСК) программа сама сканирует каждые 30 мин и шлёт **новые** сэтапы в Telegram
