        ...
        if last["Close"] >= last["HH"] and prev["Close"] < prev["HH"]:
            ...
            return [
                Setup(
                    ...
                    entry=round(entry, 2),
                    stop=round(stop, 2),
                    exit_rule=f"Трейлинг-стоп {p['atr_mult']}×ATR (или по структуре)",
                    atr=round(float(last["ATR"]), 2),
                    score=score + 20,  # bonus for pure breakout
                    reason=f"Новый максимум закрытия за {p['lookback']} дней на уровне {entry:.2f}. Участие в тренде (Rayner Trend Following).",
        ...
        if last["Close"] > prev["DonchianHigh"] and prev["Close"] <= prev["DonchianHigh"]:
            ...
            return [
                Setup(
                    ...
                    entry=round(entry, 2),
                    stop=round(stop, 2),
                    exit_rule=f"Выход: закрытие ниже минимума за {p['exit_period']} дней (выход Дончиана)",
                    atr=round(float(last["ATR"]), 2),
                    score=score + 15,
                    reason=f"Пробой максимума канала Дончиана за {p['entry_period']} дней. Вход в стиле Turtle / тренд.",
                    suggested_shares=size,
                    risk_amount=round(risk_amount, 2),
                    capital_pct=p["risk_pct"],
                )
