...
| Режим (`universe`) | Что сканирует |
|--------------------|---------------|
| `sample` | 25 ликвидных акций TQBR |
| `blue` | ~37 голубых фишек |
| `futures` / `forts` | Ликвидные фьючерсы FORTS (автовыбор ближайшего контракта по OI) |
| `mixed` | Акции sample + фьючерсы |
| `tqbr` / `full` | Расширенный список акций |

### Фьючерсы (FORTS) — автообновление контракта
Каждый запуск программа сама выбирает **ближайший ликвидный** контракт по открытому интересу:
...
- **GOLD** — золото  
- **NG** — природный газ  
- **Eu / ED** — EUR/RUB, EUR/USD  
- **CNY** — CNY/RUB  
- **SBRF, GAZR, LKOH, ROSN, VTBR** — фондовые фьючерсы  

Пример: сегодня Si → `SiU6` (сентябрь 2026). После экспирации автоматически переключится на следующий.
